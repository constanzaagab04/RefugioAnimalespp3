...
// INCLUDES CORREGIDOS Y AÑADIDOS
...
void pausarYLimpiar() {
    ...
    // 1. Limpia cualquier estado de error o flag (ej. failbit)
    if (cin.fail()) {
        cin.clear();
    }
...
// 1. Dibuja el menú centrado, resaltando la opción actual
...
// -------------------------------------------------------------
// LOGICA DE NEGOCIO (ejecutarMenu) - CORRECCIONES EN I/O
// -------------------------------------------------------------
            ...
            case 1: { // GESTIÓN DE ANIMALES
                ...
                if (op == 1) { // Listar todos
                    ...
                    if (!(cin >> id)) {
                        ...
                        limpiarBuffer(); // Limpia el buffer (AÑADIDO)
                        pausarYLimpiar();
                        break;
                    }
                    limpiarBuffer();
...
#include <locale.h>
#include <cstdlib> // Necesario para system()
#include <cstdio>  // NECESARIO para usar getchar()

// Librería para el menú interactivo
#include <ncurses.h>


// INCLUDES (Asegúrate que estas rutas y archivos existan en tu proyecto)
#include "Base/Animal.h"
#include "Utilidad/Enums.h"
#include "Core/Refugio.h"
#include "Dominio/Perro.h"
...
// -------------------------------------------------------------
// FUNCIONES DE UTILIDAD: PAUSA ROBUSTA Y LIMPIEZA (FINAL SIN system("clear"))
// -------------------------------------------------------------
...
void pausarYLimpiar() {
    ...
    // 3. Espera CRÍTICA: Se usa getchar() para mayor fiabilidad.
    getchar();

    // NOTA: Se ELIMINÓ system("clear") para evitar conflictos de renderizado.
}
...
void show_message_box(const string& message) {
    ...
    int ch;
    // Espera la pulsación de ENTER
    while ((ch = wgetch(message_win)) != 10 && ch != '\n');

    delwin(message_win);
    endwin();

    // CORRECCIÓN CRÍTICA: Limpiar el buffer después de salir de ncurses
    cin.clear();
    cin.ignore(numeric_limits<streamsize>::max(), '\n');

    // NOTA: Se ELIMINÓ system("clear")
}
...
int run_menu(const vector<string>& options, const string& title) {
    ...
    // CRÍTICO: clear() de ncurses es la limpieza de pantalla
    clear();
    noecho();
    cbreak();
    curs_set(0);
